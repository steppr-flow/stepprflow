...
class WorkflowExecutionTest {
    ...
    class LombokGeneratedTests {
        ...
        @Test
        @DisplayName("Should support no-args constructor and all getters")
        void shouldSupportNoArgsConstructorAndGetters() {
            WorkflowExecution execution = new WorkflowExecution();
            assertThat(execution.getVersion()).isNull();
            assertThat(execution.getInitiatedBy()).isNull();

            execution.setVersion(1L);
            execution.setInitiatedBy("admin");
            assertThat(execution.getVersion()).isEqualTo(1L);
            assertThat(execution.getInitiatedBy()).isEqualTo("admin");
        }
        ...
        void shouldSupportToString() {
            WorkflowExecution execution = WorkflowExecution.builder()
                    .executionId("exec-1")
                    .build();
            assertThat(execution.toString()).contains("exec-1");
        }

        @Test
        @DisplayName("Should support equals and hashCode")
        void shouldSupportEqualsAndHashCode() {
            WorkflowExecution exec1 = WorkflowExecution.builder()
                    .executionId("exec-1")
                    .build();
            WorkflowExecution exec2 = WorkflowExecution.builder()
                    .executionId("exec-1")
                    .build();
            assertThat(exec1).isEqualTo(exec2);
            assertThat(exec1.hashCode()).isEqualTo(exec2.hashCode());
        }

        @Test
        @DisplayName("Should support hasAttempts")
        void shouldSupportHasAttempts() {
            WorkflowExecution execution = WorkflowExecution.builder()
                    .executionId("exec-1")
                    .build();
            assertThat(execution.hasAttempts()).isFalse();

            ArrayList<WorkflowExecution.ExecutionAttempt> attempts = new ArrayList<>();
            attempts.add(WorkflowExecution.ExecutionAttempt.builder().attemptNumber(1).build());
            execution.setExecutionAttempts(attempts);
            assertThat(execution.hasAttempts()).isTrue();
        }

        @Test
        @DisplayName("Should support setStepHistory defensive copy")
        void shouldSupportSetStepHistoryDefensiveCopy() {
            WorkflowExecution execution = WorkflowExecution.builder()
                    .executionId("exec-1")
                    .build();

            ArrayList<WorkflowExecution.StepExecution> history = new ArrayList<>();
            history.add(WorkflowExecution.StepExecution.builder().stepId(1).build());
        ...
        @Test
        @DisplayName("StepExecution should support no-args constructor and stepLabel getter")
        void stepExecutionShouldSupportNoArgsAndStepLabel() {
            WorkflowExecution.StepExecution step = new WorkflowExecution.StepExecution();
            assertThat(step.getStepLabel()).isNull();

            step.setStepLabel("Validate");
            assertThat(step.getStepLabel()).isEqualTo("Validate");
        }

        @Test
        @DisplayName("PayloadChange should support no-args constructor and data methods")
        void payloadChangeShouldSupportNoArgsAndDataMethods() {
            WorkflowExecution.PayloadChange change = new WorkflowExecution.PayloadChange();
            assertThat(change.getFieldPath()).isNull();
            assertThat(change.toString()).isNotNull();
            assertThat(change).isEqualTo(new WorkflowExecution.PayloadChange());
            assertThat(change.hashCode()).isEqualTo(new WorkflowExecution.PayloadChange().hashCode());
        }

        @Test
        @DisplayName("ExecutionAttempt should support no-args constructor and all methods")
        void executionAttemptShouldSupportNoArgsAndAllMethods() {
            WorkflowExecution.ExecutionAttempt attempt = new WorkflowExecution.ExecutionAttempt();
            assertThat(attempt.getAttemptNumber()).isZero();
            assertThat(attempt.toString()).isNotNull();
            assertThat(attempt).isEqualTo(new WorkflowExecution.ExecutionAttempt());
            assertThat(attempt.hashCode()).isEqualTo(new WorkflowExecution.ExecutionAttempt().hashCode());
        }

        @Test
        @DisplayName("ExecutionAttempt should support setPayloadChanges defensive copy")
        void executionAttemptShouldSupportSetPayloadChanges() {
            WorkflowExecution.ExecutionAttempt attempt = WorkflowExecution.ExecutionAttempt.builder()
                    .attemptNumber(1)
                    .build();

            ArrayList<WorkflowExecution.PayloadChange> changes = new ArrayList<>();
            changes.add(WorkflowExecution.PayloadChange.builder().fieldPath("f1").build());
            attempt.setPayloadChanges(changes);
            assertThat(attempt.getPayloadChanges()).hasSize(1);

            attempt.setPayloadChanges(null);
            assertThat(attempt.getPayloadChanges()).isEmpty();
        }
    }
}